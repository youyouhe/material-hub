  ...
  return (
    <div className="w-[400px] bg-cp-card border-l border-cp-border shrink-0 overflow-y-auto max-h-[calc(100vh-6rem)]">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-cp-border sticky top-0 bg-cp-card z-10">
        ...
        <button onClick={onClose} className="p-1 rounded cp-hover">
      ...
      <div className="p-4 space-y-4">
        ...
        <div>
          {editing ? (
            <div className="flex gap-2">
              ...
              <button onClick={() => { setEditing(false); setEditTitle(doc.title); }} className="p-1 text-cp-dim cp-hover rounded"><XCircle className="w-4 h-4" /></button>
            </div>
          ) : (
            <div className="flex items-start gap-2">
              <h3 className="text-lg font-semibold text-cp-text flex-1">{doc.title}</h3>
              {canEdit && (
                <button onClick={() => setEditing(true)} className="p-1 text-cp-dim cp-hover rounded shrink-0">
                    ...
                    {Object.entries(extracted).map(([key, value]) => {
                      ...
                      return (
                        <div key={key} className="flex justify-between text-sm gap-2">
                          ...
                          <span className="text-cp-text text-right break-all">{<RenderValue value={value} />}</span>
                        </div>
                      );
                    })}
                  </div>
        ...
        {doc.entities && doc.entities.length > 0 && (
          <div>
            ...
            <button
              ...
              className="mt-2 text-xs text-cp-purple-light hover:text-cp-purple flex items-center gap-1"
            >
              🔗 查看知识图谱
            </button>
          </div>
        ...
        {/* Hint for docs without entities */}
        {(!doc.entities || doc.entities.length === 0) && (
          <div className="text-xs text-cp-dim mt-2">
            💡 该文档暂无关联实体，可在审核页面关联后查看知识图谱
          </div>
        )}
