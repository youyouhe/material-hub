    ...
    <div>
      ...
      >
        {renaming ? (
          <div className="flex items-center gap-1 px-2 py-0.5" style={{ paddingLeft: `${depth * 16 + 8}px` }}>
            ...
            <button onClick={() => setRenaming(false)} className="p-0.5 text-cp-dim cp-hover rounded"><X className="w-3 h-3" /></button>
          </div>
        ) : (
          <button
            onClick={() => {
            ...
            className={clsx(
              'flex items-center gap-1 w-full text-left px-2 py-1 text-sm rounded cp-hover transition-colors text-cp-muted',
          ...
          >
            ...
            {(node.doc_count ?? 0) > 0 && (
              <span className="ml-auto shrink-0 text-xs text-cp-dim bg-cp-purple/5 rounded-full px-1.5 py-0.5 min-w-[1.25rem] text-center">
                {node.doc_count}
              </span>
            )}
          </button>
          ...
          <div className="absolute right-1 top-1/2 -translate-y-1/2 flex items-center gap-0.5 bg-cp-card/90 rounded px-0.5">
            {siblingIndex > 0 && (
              <button
                ...
                title="上移"
                className="p-0.5 text-cp-dim hover:text-cp-cyan rounded cp-hover"
              >
            ...
            <button
              ...
              title="重命名"
              className="p-0.5 text-cp-dim hover:text-cp-purple-light rounded cp-hover"
            >
            ...
            <button
              ...
              title="删除"
              className="p-0.5 text-cp-dim hover:text-red-400 rounded cp-hover"
            >
      ...
      {adding && (
        <div className="flex items-center gap-1 px-2 py-0.5" style={{ paddingLeft: `${(depth + 1) * 16 + 8}px` }}>
          ...
          <button onClick={() => { setAdding(false); setNewName(''); }} className="p-0.5 text-cp-dim cp-hover rounded"><X className="w-3 h-3" /></button>
        </div>
      )}
  ...
  return (
    <div className="py-2">
      ...
      <button
        ...
        className={clsx(
          'flex items-center gap-1.5 w-full text-left px-3 py-1 text-sm rounded cp-hover transition-colors text-cp-muted',
      ...
      {addingRoot && (
        <div className="flex items-center gap-1 px-3 py-0.5">
          ...
          <button onClick={() => { setAddingRoot(false); setNewRootName(''); }} className="p-0.5 text-cp-dim cp-hover rounded"><X className="w-3 h-3" /></button>
        </div>
      )}
