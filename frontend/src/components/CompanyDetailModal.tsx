...
export default function CompanyDetailModal({ entity, onClose, onRefresh }: Props) {
  ...
  const loadDocs = async () => {
    setLoading(true);
    try {
      const data = await searchDocuments({ entity_id: entity.id, limit: 100 });
      setDocs(data.results);
    } catch (err) { toast.error('加载文档失败'); }
  ...
  return (
    <>
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                ...
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                  {attrs.legal_person && <div><span className="text-gray-500">法定代表人:</span> <span className="text-gray-900">{String(attrs.legal_person)}</span></div>}
                  {attrs.credit_code && <div><span className="text-gray-500">信用代码:</span> <span className="text-gray-900 font-mono">{String(attrs.credit_code)}</span></div>}
                </div>
                {attrs.address && <div className="mt-2 text-sm"><span className="text-gray-500">地址:</span> <span className="text-gray-900">{String(attrs.address)}</span></div>}
              </div>
             ...
             : <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {materials.map((m: any) => (
                  <MaterialCard key={m.id} material={m}
                    ...
                    onTriggerOCR={async () => { toast('OCR 功能已集成到上传管线'); }}
      ...
      {showPicker && <MaterialPicker onSelect={async () => { setShowPicker(false); loadDocs(); onRefresh(); }} onClose={() => setShowPicker(false)} excludeIds={docs.map(d => d.id)} />}
