...
export interface KbGraphResponse {
  ...
  relations: Array<{ direction?: string; target_id?: number; source_id?: number; from_id?: number; to_id?: number; relation: string }>;
  related_entities: KbGraphEntity[];
  events: Array<{ id: number; title: string; event_type: string; event_date?: string; doc_id?: number }>;
  depth2_entities?: KbGraphEntity[];
}
...
export interface KbSyncResult {
  sync: {
    entities: { synced: number; skipped: number; errors: number };
