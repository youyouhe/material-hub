...
import { FileText, Filter, ChevronLeft, ChevronRight, Eye, Folder, Brain, Pencil } from 'lucide-react';
...
interface ClassifyState { docId: number; docTypes: any[]; folders: any[] }

export default function DocumentsPage({ folderId, selectedDocumentId, onSelectDocument, userRole }: Props) {
  ...
  const openClassify = async (docId: number) => {
    try {
      ...
      const allTypes: any[] = [];
      Object.values(dt.doc_types || {}).forEach((arr: any) => (arr as any[]).forEach((t: any) => allTypes.push(t)));
      const flatFolders: any[] = [];
      const walk = (nodes: any[]) => { for (const n of nodes) { flatFolders.push(n); if (n.children) walk(n.children); } };
      walk(ft);
      setClassify({ docId, docTypes: allTypes, folders: flatFolders });
    } catch { toast.error('加载分类选项失败'); }
  };

  const doClassify = async (docId: number, docTypeId: number, folderId: number) => {
    try {
      await updateDocument(docId, { doc_type_id: docTypeId, folder_id: folderId });
      toast.success('已分类');
      setClassify(null);
      fetchDocuments();
    } catch (err: any) { toast.error(`分类失败: ${err.message}`); }
  };
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm">
                        {doc.doc_type ? (
                          ...
                          classify?.docId === doc.id ? (
                            <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                              <select
                                onChange={e => { const [tid, fid] = e.target.value.split(','); doClassify(doc.id, Number(tid), Number(fid)); }}
                                className="text-xs rounded border border-cp-border bg-cp-input text-cp-text px-1 py-0.5"
                                autoFocus
                              >
                                <option value="">选择类型+文件夹</option>
                                {classify.docTypes.map((dt: any) =>
                                  classify.folders.map((f: any) => (
                                    <option key={`${dt.id}-${f.id}`} value={`${dt.id},${f.id}`}>
                                      {dt.name} → {f.path}
                                    </option>
                                  ))
                                )}
                              </select>
                              <button onClick={() => setClassify(null)} className="text-cp-dim hover:text-cp-text text-xs">✕</button>
                            </div>
                          ) : (
                            <button onClick={(e) => { e.stopPropagation(); openClassify(doc.id); }}
                              className="text-cp-dim hover:text-cp-purple-light text-xs flex items-center gap-1 cp-hover px-1.5 py-0.5 rounded"
                              title="点击分类">
                              <Pencil className="w-3 h-3" /> 未分类
                            </button>
                          )
                        ) : (
                        ...
                        {(() => {
                          ...
                          return (
                            <button
                              ...
                              className="p-1 text-cp-dim hover:text-cp-cyan transition-colors rounded cp-hover"
                              title="预览文件"
                            >
