...
import { uploadFile, getUploadQueue, approveUpload, rejectUpload, listDocTypes, getFolderTree, getApproveSuggestions } from '../services/api-v2';
...
export default function UploadPageV2({ userRole }: UploadPageV2Props) {
  ...
  const [approveForm, setApproveForm] = useState({ title: '', folder_id: 0, doc_type_id: 0, kb_index: false });
  ...
  const handleApprove = async () => {
    ...
    try {
      await approveUpload(approveModal, {
        ...
        kb_index: approveForm.kb_index,
      });
      toast.success(approveForm.kb_index ? '已批准 (KB索引后台进行中)' : '已批准');
                      ...
                      <td className="px-4 py-3 text-right space-x-2">
                        ...
                        <button
                          onClick={() => {
                            ...
                            setApproveForm({ title: item.title, folder_id: 0, doc_type_id: 0, kb_index: false });
                            setSuggestions(null);
                            getApproveSuggestions(item.id).then(setSuggestions).catch(() => {});
                          }}
        ...
        <div className="cp-overlay fixed inset-0 flex items-center justify-center z-50">
          <div className="cp-card rounded-lg p-6 w-full max-w-md">
            ...
            <div className="space-y-3">
              ...
              {/* AI suggestions */}
              {suggestions && (suggestions.doc_type || suggestions.folder || (suggestions as any).material_type) && (
                <div className="bg-cp-purple/5 border border-cp-purple/20 rounded-lg p-2 text-xs space-y-1">
                  <span className="text-cp-purple-light font-medium">💡 AI 建议 {(suggestions as any).confidence ? `(${Math.round((suggestions as any).confidence * 100)}%)` : ''}</span>
                  {(suggestions as any).material_type && !suggestions.doc_type && (
                    <div className="text-cp-muted">
                      🏷️ AI 识别类型: <span className="text-cp-text">{(suggestions as any).material_type}</span>
                      <span className="text-cp-dim"> (无匹配类型，请下拉选择或新建)</span>
                    </div>
                  )}
                  {suggestions.doc_type && !approveForm.doc_type_id && (
                    <button onClick={() => setApproveForm(p => ({ ...p, doc_type_id: suggestions.doc_type!.id }))}
                  ...
                  {suggestions.folder && !approveForm.folder_id && (
                    <button onClick={() => setApproveForm(p => ({ ...p, folder_id: suggestions.folder!.id }))}
                      className="block w-full text-left text-cp-text hover:text-cp-purple-light">
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-cp-divider">
              <input type="checkbox" id="kb_index_check"
                checked={approveForm.kb_index}
                onChange={(e) => setApproveForm({ ...approveForm, kb_index: e.target.checked })}
                className="w-4 h-4 rounded border-cp-border text-cp-purple" />
              <label htmlFor="kb_index_check" className="text-sm text-cp-muted cursor-pointer">
                🧠 AI知识库索引（语义搜索+实体提取）
              </label>
            </div>
