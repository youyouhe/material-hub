...
from dms_models import get_dms_session, Entity, DocumentEntity
...
async def create_entity(data: EntityCreate):
    """Create a new entity."""
    if data.entity_type not in ("org", "person"):
        raise HTTPException(status_code=400, detail="entity_type must be 'org' or 'person'")
