...
# ============================================================
# DocType → Folder Mappings
# ============================================================

from dms_processor import get_folder_path_for_doctype, set_folder_mapping, remove_folder_mapping
...
async def list_folder_mappings():
    """List all doc_type → folder mappings (built-in + custom)."""
    from dms_processor import _BUILTIN_FOLDER_PATHS, _get_custom_folder_mappings

    with get_dms_session() as s:
        dt_list = s.query(DocType).all()
        # Extract data before session closes
        doc_types = {dt.code: {"id": dt.id, "name": dt.name, "code": dt.code} for dt in dt_list}
    ...
    for code, path in _BUILTIN_FOLDER_PATHS.items():
        ...
        builtin[code] = {
            "doc_type_code": code,
            "doc_type_name": dt["name"] if dt else code,
            "folder_path": path,
            "source": "builtin",
        }
    ...
    all_mappings = {**builtin, **custom}

    for dt in doc_types.values():
        if dt["code"] not in all_mappings:
            all_mappings[dt["code"]] = {
    ...
    return {"mappings": list(all_mappings.values())}


@router.put("/mappings/{doc_type_code}", dependencies=[require_role("admin")])
async def set_folder_mapping_endpoint(doc_type_code: str, data: MappingSet):
    """Set or override the folder mapping for a doc_type."""
    with get_dms_session() as s:
        if not s.query(DocType).filter(DocType.code == doc_type_code).first():
            raise HTTPException(status_code=404, detail=f"DocType '{doc_type_code}' not found")

    set_folder_mapping(doc_type_code, data.folder_path)
    return {"success": True, "doc_type_code": doc_type_code, "folder_path": data.folder_path}


@router.delete("/mappings/{doc_type_code}", dependencies=[require_role("admin")])
async def remove_folder_mapping_endpoint(doc_type_code: str):
    """Remove a custom folder mapping (reverts to built-in or none)."""
    remove_folder_mapping(doc_type_code)
    return {"success": True, "doc_type_code": doc_type_code}
