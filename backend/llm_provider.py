...
class LLMProvider(ABC):
    ...
    def embed(self, texts: List[str], model: str = None) -> List[List[float]]:
        """
        Generate embeddings for texts via OpenAI-compatible /v1/embeddings.

        Args:
            texts: list of text strings to embed (max ~100 per batch)
            model: embedding model name (defaults to EMBEDDING_MODEL env or text-embedding-3-small)
...
class AnthropicProvider(LLMProvider):
    ...
    def embed(self, texts: List[str], model: str = None) -> List[List[float]]:
        ...
        Falls back to the OpenAI-compatible endpoint from the same base config
        (e.g., via OpenRouter or a proxy). If that's not available,
        uses a deterministic local fallback.
        """
        # Try using the OpenAI-compatible path if available
        ...
        # Fall back to deterministic local embeddings
        logger.info("No EMBEDDING_BASE_URL set for Anthropic provider, using local fallback")
...
def _openai_embed(
    ...
    """Shared OpenAI-compatible embedding helper for all providers."""
    import time
    ...
    for attempt in range(max_retries):
        try:
            if attempt > 0:
                logger.info(f"{provider_name} embedding API retry {attempt}...")
            ...
            if len(embeddings) != len(texts):
                logger.warning(
                    f"{provider_name} embedding count mismatch: "
                    f"expected {len(texts)}, got {len(embeddings)}"
                )
                # Pad with zero vectors or trim
                dims = int(os.getenv("EMBEDDING_DIMENSIONS", "1024"))
                while len(embeddings) < len(texts):
                    embeddings.append([0.0] * dims)

            logger.info(f"{provider_name} embedding API success: {len(embeddings)} vectors")
            return embeddings
            ...
            if attempt < max_retries - 1:
                time.sleep(retry_delays[attempt])
                continue
            raise Exception(f"{provider_name} embedding API timeout after retries")
        except requests.exceptions.ConnectionError as e:
            if attempt < max_retries - 1:
                time.sleep(retry_delays[attempt])
                continue
...
def _deterministic_embedding(text: str, dimensions: int = 1024) -> List[float]:
    """Deterministic local embedding fallback using SHA-256 hash projection.

    Ported from SAG's deterministicEmbedding() in embedding-client.ts.
    Always produces the same vector for the same text input.
    """
    import hashlib
    import math

    vector = [0.0] * dimensions
    blocks_needed = (dimensions + 31) // 32  # 32 bytes per SHA-256

    for block_idx in range(blocks_needed):
        seed = f"{text}:{block_idx}".encode("utf-8")
        digest = hashlib.sha256(seed).digest()
        base_idx = block_idx * 32
        for byte_idx in range(min(32, dimensions - base_idx)):
            byte_val = digest[byte_idx]
            vector[base_idx + byte_idx] = (byte_val / 255.0) * 2.0 - 1.0
...
def get_embedding_provider() -> LLMProvider:
    """
    Get an LLMProvider configured for embedding calls.

    Uses EMBEDDING_BASE_URL / EMBEDDING_API_KEY / EMBEDDING_MODEL env vars
    if set (allows a different endpoint than the chat LLM). Otherwise falls
    back to the main LLM provider config (DB settings > env vars).

    Recommended for Chinese documents:
