...
def find_matching_document(
    ...
    """
    Find an existing document with the same entity + DocType.

    Args:
        doc_id: The newly uploaded document ID (to exclude from results)
        entity_id: The entity linked to the new document
        doc_type_code: The DocType code of the new document
        session: DB session

    Returns:
        Matching existing DmsDocument, or None
    """
    ...
    if matching_docs:
        logger.info(
            f"Found {len(matching_docs)} matching doc(s) for entity={entity_id}, "
            f"type={doc_type_code}. Using most recent: doc={matching_docs[0].id}"
        )
