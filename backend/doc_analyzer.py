...
def run_selective_ocr(doc_id: int, page_numbers: list[int], ocr_provider_override: str = None) -> dict:
    ...
    # Normalize: convert 1-indexed page numbers to 0-indexed
    normalized_pages = [p - 1 if p >= 1 else p for p in page_numbers]

    results = {}
    try:
        if mime_type.startswith("image/"):
            # Single image — always OCR page 0 regardless of input
            if 0 in normalized_pages or 1 in page_numbers:
                ...
                if cached:
                    ...
                    logger.info(f"Calling OCR for doc {doc_id}")
                    text = ocr_image(file_path, page_number=1)
                    if text:
                        save_ocr_to_cache(file_path, 0, text)
                        results[0] = text
                    else:
                        logger.warning(f"OCR returned empty text for doc {doc_id}")
            ...
            for page_num in sorted(normalized_pages):
